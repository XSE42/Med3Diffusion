...
class DualTransformer3DModel(nn.Module):
    """
    Dual transformer wrapper that combines two `Transformer3DModel`s for mixed inference.
    ...
    def forward(
        ...
        """
        Args:
            hidden_states ( When continuous, `torch.FloatTensor`
                of shape `(batch size, channel, depth, height, width)`): Input hidden_states.
        ...
        return Transformer3DModelOutput(sample=output_states)
