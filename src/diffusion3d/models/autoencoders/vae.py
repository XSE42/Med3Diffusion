...
from ..unets.unet_3d_blocks import get_down_block, get_up_block, UNetMidBlock3D
from ..attention_processor import SpatialNorm3d


@dataclass
class Decoder3DOutput(BaseOutput):
    """
...
class Encoder3D(nn.Module):
    ...
    Parameters:
        ...
        down_block_types (`Tuple[str, ...]`, *optional*, defaults to `("DownEncoderBlock3D",)`):
            The types of down blocks to use. See `.unet_3d_blocks.get_down_block` for available
            options.
...
class Decoder3D(nn.Module):
    ...
    Args:
        ...
        up_block_types (`Tuple[str, ...]`, *optional*, defaults to `("UpDecoderBlock3D",)`):
            The types of up blocks to use. See `.unet_3d_blocks.get_up_block` for available options.
        block_out_channels (`Tuple[int, ...]`, *optional*, defaults to `(64,)`):
            The number of output channels for each block.
        layers_per_block (`int`, *optional*, defaults to 2):
            The number of layers per block.
    ...
    def __init__(
        ...
        if norm_type == "spatial":
            self.conv_norm_out = SpatialNorm3d(block_out_channels[0], temb_channels)
        else:
...
class DiagonalGaussianDistribution3D(object):
    ...
    def kl(self, other: "DiagonalGaussianDistribution3D" = None) -> torch.Tensor:
        if self.deterministic:
    ...
    def nll(self, sample: torch.Tensor, dims: Tuple[int, ...] = [1, 2, 3, 4]) -> torch.Tensor:
        if self.deterministic:
            return torch.Tensor([0.0])
