...
from utils.data import ADNI_3D_NumCond_Set
...
def log_validation(
    ...
    pipeline = NumCond3DDiffusionPipeline(
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="outputs/numcond_ldm_adni3d",
...
def main():
    ...
    train_dataset = ADNI_3D_NumCond_Set(
        data_dir=args.data_dir,
        scan_list=train_meta["scan_list"],
    )

    def collate_fn(examples):
        scan = torch.stack([example["scan"] for example in examples])
        scan = scan.to(memory_format=torch.contiguous_format).float()
        num_cond = torch.stack([example["num_cond"] for example in examples])
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(unet, cond_encoder):
                ...
                batch_vols = F.interpolate(batch["scan"], size=DATA_SHAPE, mode=interpolate_mode)
                ...
                # f. get the target for loss depending on the prediction type
                ...
                # i. gather the losses across all processes for logging (if we use distributed training)
            ...
            # l. checks if the accelerator has performed an optimization step behind the scenes
            if accelerator.sync_gradients:
                if args.use_ema:
