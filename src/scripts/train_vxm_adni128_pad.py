...
from utils.data import ADNI_3D_128_Set
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train VoxelMorph on ADNI 128 dataset")
...
def main(args):
    ...
    vxm, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        vxm, optimizer, train_dataloader, lr_scheduler
    )
    ...
    if accelerator.is_main_process:
        ...
        grid_shape = (args.val_size // 2, 1, *(vxm.inshape))
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(vxm):
                # data
                batch = F.pad(batch, pad=PAD_SEQ, mode="constant", value=-1)
                if batch.shape[0] % 2 == 1:
                    batch = batch.repeat_interleave(repeats=2, dim=0)
                source, target = torch.tensor_split(batch, 2, dim=0)
                # forward
                source_warped, velocity_flow = vxm(source, target)
                # loss
                mse_loss = mse_loss_fn.loss(target, source_warped)
                grad_loss = grad_loss_fn.loss(None, velocity_flow)
                loss = mse_loss + args.grad_weight * grad_loss
                # backward
            ...
            if epoch % args.save_volume_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                with torch.no_grad():
                    for i, batch in enumerate(val_dataloader):
                        ...
                        batch = F.pad(batch, pad=PAD_SEQ, mode="constant", value=-1)
                        source, target = torch.tensor_split(batch, 2, dim=0)
                        source_warped, displacement_flow = vxm(source, target, registration=True)
                    ...
                    source = source / 2 + 0.5
                    target = target / 2 + 0.5
                    source_warped = source_warped / 2 + 0.5
                    ...
                    for batch_idx in range(source.shape[0]):
                        ...
                        sitk.WriteImage(src_img, os.path.join(sampling_dir, f"src_epoch{epoch}_id{batch_idx}.nii.gz"))
                        sitk.WriteImage(trg_img, os.path.join(sampling_dir, f"trg_epoch{epoch}_id{batch_idx}.nii.gz"))
                        sitk.WriteImage(
                            src_warped_img,
                            os.path.join(sampling_dir, f"src_warped_epoch{epoch}_id{batch_idx}.nii.gz")
                        )
                        sitk.WriteImage(
                            grid_warped_img,
                            os.path.join(sampling_dir, f"grid_warped_epoch{epoch}_id{batch_idx}.nii.gz")
                        )
                    ...
                    for idx in VAL_SLICE_IDX:
                        ...
                        tracker.add_images(f"source_slice{idx}", source_img, epoch)
                        tracker.add_images(f"target_slice{idx}", target_img, epoch)
                        tracker.add_images(f"source_warped_slice{idx}", source_warped_img, epoch)
