...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from src.utils.pipeline_numcond3d import NumCond3DDiffusionPipeline

        >>> pipe = NumCond3DDiffusionPipeline.from_pretrained("outputs/numcond_ldm_adni3d", torch_dtype=torch.float16)
        >>> pipe = pipe.to("cuda")

        >>> cond = torch.tensor([75.0, 1.0], device="cuda")
        >>> image = pipe(cond).images[0]
        ```
"""
...
@dataclass
class NumCond3DDiffusionOutput(BaseOutput):
    """
    Output class for NumCond 3D Diffusion pipelines.
...
class NumCond3DDiffusionPipeline(DiffusionPipeline, LoraLoaderMixin, FromSingleFileMixin):
    r"""
    ...
    def __call__(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`~pipeline_numcond3d.NumCond3DDiffusionOutput`] instead of a
                plain tuple.
        ...
        Returns:
            [`~pipeline_numcond3d.NumCond3DDiffusionOutput`] or `tuple`:
                If `return_dict` is `True`, [`~pipeline_numcond3d.NumCond3DDiffusionOutput`] is returned,
        ...
        return NumCond3DDiffusionOutput(volumes=vol)
